...
m = models._LSTM2(in_dim = in_dim, hid_dim = hid_dim, out_dim = out_dim, num_layers = 1).cuda()
...
for e in range(num_epoch):
    ...
    for _, samples in enumerate(train_loader): 
        ...
        y_train.to(device).reshape(-1,6).size()
        
        out.size()
        y_train.to(device).size()
    ...
    with torch.no_grad():
        for idx, samples in enumerate(test_loader):
            ...
            #앞에 세개는 lat, 뒤에 세개는 long 
            lat = scaler[0].inverse_transform(out[:,:3].reshape(-1,1))
            long = scaler[1].inverse_transform(out[:,3:].reshape(-1,1)) 
        ...
        for r in range(X_test.size(0)): #전체 데이터 돌면서
            #3개씩 쪼개면서
            a = (pred.iloc[r*3:r*3+3]['latitude'].reset_index(drop=True), pred.iloc[r*3:r*3+3]['longitude'].reset_index(drop=True))
            b = (real.iloc[r*3:r*3+3]['latitude'].reset_index(drop=True), real.iloc[r*3:r*3+3]['longitude'].reset_index(drop=True))
            dis_sum = []
            for x in range(p_size): #3개의 오차거리가 나옴
            
                a_ = (a[0][x], a[1][x])
                b_ = (b[0][x], b[1][x])
                
                dis = haversine(a_,b_, unit='m')
                dis_sum.append(dis)
            sum_dis = np.mean(dis_sum) #3개 오차거리 평균 
            dis_diff.append(sum_dis)
