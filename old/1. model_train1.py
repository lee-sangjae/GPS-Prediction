...
import src.attention2 as attention
attn = attention.Attention(enc_hid_dim, dec_hid_dim).to(device)

enc = attention.Encoder(in_dim, enc_hid_dim, dec_hid_dim, n_layers, act, 0.3 ).to(device)
dec = attention.Decoder(2, out_dim, enc_hid_dim, dec_hid_dim, 0.3, attn).to(device)


model['Attention'] = attention.Seq2Seq(enc, dec, device).to(device)
    ...
    for e in tqdm(range(1, num_epoch+1)):
            ...
            if (md == 'RNN') | (md=='LSTM'):
                ...
                for _, samples in enumerate(data_loader['train']): 
                    ...
                    #running_train_loss+= train_loss.cpu().numpy()와 동일 
                ...
                for _, samples in enumerate(data_loader['train']): 
                    ...
                    train_out, _ = m(X_train, y_train, 0.5)
                    
                    train_out = train_out.swapaxes(0,1)
                    y_train = y_train.swapaxes(0,1)
                ...
                with torch.no_grad():
                    for _, samples in enumerate(data_loader['valid']): 
                        ...
                        val_out, attention_w = m(X_val, y_val,  0.5)
                ...
                attention_w = pd.DataFrame(attention_w, columns = ['seq1', 'seq2', 'seq3', 'seq4', 'seq5'] )
                w_result = []
                for i in attention_w.columns:
                    w_result.append(attention_w[i])
            
            
                #val loss가 감소하면 모델 저장 
                if min_val_loss > val_loss:
                        min_val_loss = val_loss
                        torch.save(m, f'./'+md+'({}-{})'.format(w_size,p_size)+'128128.pt')     
