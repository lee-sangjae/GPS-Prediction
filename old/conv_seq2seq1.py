...
class Encoder(nn.Module):
    def __init__(self, input_dim, hid_dim, n_layers, dropout):
        ...
        self.conv1 = nn.Conv1d(in_channels = input_dim, out_channels = 2, kernel_size = 2, stride = 1)
        self.conv2 = nn.Conv1d(in_channels = 2, out_channels = 2, kernel_size = 2, stride = 1)
        self.conv3 = nn.Conv1d(in_channels = 2, out_channels = 2, kernel_size = 2, stride = 1)
    
    
        self.rnn = nn.LSTM(2, hid_dim, n_layers, dropout=dropout, batch_first=True)
    ...
    def forward(self, x):
        ...
        h_0 = Variable(torch.zeros(self.n_layers, feature_map.size(0), self.hid_dim)).to(device) #은닉상태 0 초기화
        c_0 = Variable(torch.zeros(self.n_layers, feature_map.size(0), self.hid_dim)).to(device) #셀 상태 0 초기화 
        
        outputs, (hidden, cell) = self.rnn(feature_map, (h_0, c_0))
