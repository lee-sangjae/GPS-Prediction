...
class RNN(nn.Module):
    def __init__(self,  in_dim, hid_dim, out_dim, num_layers, seq_length):
        ...
        self.fc_1 = nn.Linear(self.hid_dim*self.seq_length, out_dim)
    ...
    def forward(self, x):
        ...
        #initial hidden
        h_0 = Variable(torch.zeros(self.num_layers, x.size(0), self.hid_dim)).to(device) #은닉상태 0 초기화
        
        output, hn = self.rnn(x, h_0)
        
        output = output.reshape(-1, self.hid_dim*self.seq_length).to(device)
