...
result_dir = os.path.join(parent_dir, 'GPS\\pred')
...
present = pd.read_csv('C:/Users/USER/Dropbox/project code/GPS/present.csv')
real = pd.read_csv('C:/Users/USER/Dropbox/project code/GPS/real.csv')
...
for result in result_list:
    ...
    model[model_name] = pd.read_csv(os.path.join(result_dir,result))

model['1.RNN'] = model['RNN']
del(model['RNN'])

model['2.LSTM'] = model['LSTM']
del(model['LSTM'])

model['3.GRU'] = model['GRU']
del(model['GRU'])

model['4.Conv-LSTM'] = model['Conv-LSTM']
del(model['Conv-LSTM'])

model['5.Seq2Seq'] = model['Seq2Seq']
del(model['Seq2Seq'])

model['6.Conv-Seq2Seq'] = model['Conv-Seq2Seq']
del(model['Conv-Seq2Seq'])


model['7.Attention'] = model['Attention']
del(model['Attention'])
...
plt.legend(labels=['RNN', 'LSTM', 'GRU', 'Conv-LSTM', 'Seq2Seq', 'Conv-Seq2Seq', 'Attention'])     
ax.set(title = 'model sequence average error')
plt.show()

fig, ax = plt.subplots(1,1, figsize=(10,8))
sns.barplot( x = ['RNN', 'LSTM', 'GRU', 'Conv-LSTM', 'Seq2Seq', 'Conv-Seq2Seq', 'Attention'], y=diff )
