...
out_dim = 2
#encoder = seq2seq.Encoder(input_dim = in_dim, hid_dim=hid_dim, n_layers=1, dropout=0.2,).to(device)
#decoder = seq2seq.Decoder(input_dim =2,  output_dim=out_dim, hid_dim=hid_dim, n_layers=1, dropout=0.2).to(device)
#model['Seq2Seq'] = seq2seq.Seq2Seq(encoder, decoder, device).to(device)

import src.conv_seq2seq as conv_seq2seq
encoder = conv_seq2seq.Encoder(input_dim = in_dim, hid_dim=hid_dim, n_layers=1, dropout=0.2,).to(device)
decoder = conv_seq2seq.Decoder(input_dim =2,  output_dim=out_dim, hid_dim=hid_dim, n_layers=1, dropout=0.2).to(device)
