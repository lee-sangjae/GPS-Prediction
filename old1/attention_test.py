# -*- coding: utf-8 -*-
"""
Created on Fri Aug  5 10:10:55 2022
...
class Encoder(nn.Module):
    ...
    def forward(self, x):
        ...
        #c_0 = Variable(torch.zeros(self.n_layers, x.size(1), self.enc_hid_dim)).to(device) #셀 상태 0 초기화 
...
"""
hidden = torch.zeros(1, 32,256) #인코더에서 나온 히든
enc_outputs = torch.ones(5, 32, 128) #어텐션 용 

attn = nn.Linear((128+256),256)

v = nn.Linear(256, 1, bias = False)

hidden = hidden.squeeze(0).unsqueeze(1).repeat(1, 5, 1)

enc_outputs = enc_outputs.permute(1, 0, 2) #얘도 배치가 맨앞으로 오게 변경 

energy = torch.tanh(attn(torch.cat((hidden, enc_outputs), dim=2)))

attention = v(energy).squeeze(2)

F.softmax(attention, dim=1).size()


#attention = Attention(128,256)
attention(hidden, enc_outputs)
""" 
...
input = torch.zeros(32,2)
hidden = torch.ones(1, 32, 256)
enc_outputs = torch.ones(5, 32, 128)

attn = Attention(128, 256)

attention = attn(hidden, enc_outputs)
...
lstm = nn.GRU(130, 256, num_layers = 1)
 
output, hidden = lstm(rnn_input, hidden)
...
fc_out = nn.Linear((128 + 256 + 2), 2)

prediction = fc_out(torch.cat((input, output, weighted), dim=1))
...
class Decoder(nn.Module):
    ...
    def forward(self, input, hidden, enc_outputs):
        ...
        return prediction, hidden   # [배치, 출력], [시퀀스(1), 배치, 디코더 히든]
    
"""    
dec = Decoder(2, 2, 128, 256, 0.3, attention)

prediction, hidden = dec(input, hidden, enc_outputs)    
"""  
...
class Seq2Seq(nn.Module):
    ...
    def forward(self, src , trg, teacher_forcing_ratio=0.5):
        ...
        return outputs #[출력 시퀀스, 배치, 출력 변수]
