...
for folder in folder_list:
    ...
    for yt in YT_dict[folder]:
        ...
        #축 변경
        X_s = X_s.swapaxes(0,1)
        y_s = y_s.swapaxes(0,1)
...
for folder in folder_list:
    ...
    for i in ['X','y']:
        if i=='X':
            data_set[folder][i] = np.zeros((w_size,1,len(df_dict[folder].columns)))
        else:
            data_set[folder][i] = np.zeros((p_size,1,2))
    
for folder in folder_list:
    for ts in ts_dict[folder]:
        
        
        data_set[folder]['X'] = np.concatenate((data_set[folder]['X'],ts_dict[folder][ts][0]), axis=1)
        data_set[folder]['y'] = np.concatenate((data_set[folder]['y'],ts_dict[folder][ts][1]), axis=1)
...
for md in model:
    ...
    for e in tqdm(range(1, num_epoch+1)):
            ...
            elif ('Seq2Seq' in md) | ('Attention' in md):
                out_dim = 2
