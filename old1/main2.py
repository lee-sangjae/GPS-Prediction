...
for e in range(1, num_epoch+1):
    ...
    if min_val_loss > val_loss:
            ...
            torch.save(m.state_dict(),'best_seq2seq4.pth')      
    ...
    with torch.no_grad():
        #샘플링 난수 
        #ran = random.randrange(0, len(real))
        present = np.array([])
        for idx, samples in enumerate(test_loader):
            if idx%200==0:
                ...
                #나중에 시각화 할 현재 위치 
                present_lat = scaler[0].inverse_transform(X_test[ran,:,[0]].detach().cpu().numpy())
                present_long = scaler[1].inverse_transform(X_test[ran,:,[1]].detach().cpu().numpy())
                ...
                for i in range(1, p_size+1):
                    ...
                    b = real.loc[idx, ['lat'+str(i),'long'+str(i)]]
            ...
            #테스트 시각화 
                ...
                #기존 경로 시각화 
                sns.lineplot(data=v, x=0, y=1, marker='o',markersize = 10, ax=ax, estimator = None, color='g')
                
                #예측 값 하나 샘플링 
                lat = np.array(pred.iloc[ran, list(range(0,len(columns),2))]) 
                long = np.array(pred.iloc[ran, list(range(1,len(columns)+1,2))]) 
                ...
                #실제 값 하나 샘플링 
                lat = np.array(real.iloc[idx, list(range(0,len(columns),2))]) 
                long = np.array(real.iloc[idx, list(range(1,len(columns)+1,2))]) 
